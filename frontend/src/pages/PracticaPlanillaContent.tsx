...
function getGroupsForSubject(options: PracticeDesignationOption[], subject: string): PracticeDesignationOption[] {
  ...
  return options.filter((o) => {
    ...
    const key = getSubjectOptionKey(o)
  ...
  return rows.flatMap<ExcludedDay>((row) => {
    if (row.scope === 'subject') {
      ...
      return selections.map((s) => ({
        ...
        semester_id: s.semester || undefined,
        reason: row.reason,
      }))
    }
...
function hydrateExclusionRows(excludedDays: ExcludedDay[]): ExclusionRow[] {
  ...
  for (const excluded of excludedDays) {
    ...
    const hasSelection = subjectSelections.some(s => (
      s.subject === subject
      && s.group_code === group
      && s.semester === semester
    ))
                              ...
                              <div className="flex flex-wrap gap-1">
                                {((row.subjectSelections ?? []) as PracticeDesignationOption[]).map((s) => (
                                  <span key={getSubjectOptionKey(s)} className="inline-flex rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 border border-amber-100">
                                    {s.subject} ({s.group_code}, Sem. {s.semester || 'legacy'})
                                  </span>
                                ))}
                              </div>
                            )}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {item.start_date && item.end_date
                          ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-600/8 text-emerald-700 text-xs font-semibold border border-emerald-600/15">
                              <Calendar size={11} />
                        ...
                        {item.file_path ? (
                          <div className="flex items-center gap-1 flex-wrap">
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-emerald-700 bg-emerald-50 hover:bg-emerald-600 hover:text-white border border-emerald-200 hover:border-emerald-600 text-xs font-semibold transition-all duration-200 shadow-sm hover:shadow-md"
                            >
                              <Download size={12} />
                              Excel
                            </button>
                            <button
                              ...
                              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[#1C398E] bg-[#1C398E]/5 hover:bg-[#1C398E] hover:text-white border border-[#1C398E]/20 hover:border-[#1C398E] text-xs font-semibold transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50"
                            >
